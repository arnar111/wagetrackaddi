...
<style>
    ...
    .login-box {
        padding: 50px; border-radius: 10px; background-color: white; 
...
# --- CONSTANTS ---
# WE USE THE ID FROM YOUR URL NOW - MUCH SAFER
...
def get_data(worksheet_name):
    """Fetch data using ID instead of Name"""
    client = get_gsheet_client()
    if not client: return pd.DataFrame()
    try:
        # OPEN BY KEY (The Silver Bullet Fix)
        ...
        # st.error(f"Error opening '{worksheet_name}': {e}") 
        return pd.DataFrame()

def append_row(worksheet_name, row_data):
    """Append data using ID instead of Name"""
    client = get_gsheet_client()
...
def check_login(staff_code):
    ...
    # 2. Debugging Check
    if df_users.empty:
        # If empty, we check if we can even connect to the sheet
        try:
            client = get_gsheet_client()
            sheet = client.open_by_key(SHEET_ID)
            # Try to list worksheets to prove connection works
            ws_list = [ws.title for ws in sheet.worksheets()]
            if "Users" not in ws_list:
                st.error(f"⚠️ Tenging tókst en fann ekki flipann 'Users'. Flipar sem fundust: {ws_list}")
            else:
                st.error("⚠️ Fann flipann 'Users' en hann virðist vera tómur (eða vantar headers í línu 1).")
        except Exception as e:
            st.error(f"⚠️ Gat ekki opnað skjalið. Er 'launa-bot' örugglega Editor? Villa: {e}")
        return None
...
if not st.session_state.logged_in:
    ...
    with c2:
        ...
        with st.form("login_form"):
            ...
            if submitted:
                st.cache_resource.clear() # Force refresh
...
if menu == "🔥 Dagurinn í dag":
    ...
    with st.container():
        with st.form("add_sale"):
            ...
            with c1: amt = st.number_input("Upphæð", step=1000)
            with c2: note = st.text_input("Skýring")
...
# --- 2. STATS ---
    ...
    if not df_wages.empty and 'WageMonth' in df_wages.columns:
        ...
        else: st.info("Engin gögn fundust.")
    else: st.info("Engin gögn fundust.")
